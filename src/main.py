...
class Main:
    ...
    def visualize_watermark(self, dataset_name: str) -> tuple[tuple[list, list], tuple[list, list], tuple[list, list], tuple[list, list]]:
        ...
        watermarked_graph_max = inject_chain(dataset[graph_index_max], target_chain_length, is_binary, rng)
        
        # Shortest benign chain and longest watermarked chain injected
        watermarked_graph_min = inject_chain(dataset[graph_index_min], target_chain_length, is_binary, rng)
