...
class UtilityFunctions:
    ...
    def load_dataset(self, root="data/REDDIT-BINARY", name="REDDIT-BINARY", use_node_attr=True):
        """Returns TU graph dataset."""
        return TUDataset(root=f"{root}", name=f"{name}", use_node_attr=use_node_attr)
    ...
    # ------------------------------------------------------------------
    # CommonRoad / temporal graph helpers
    # ------------------------------------------------------------------
    ...
    def build_neighbors(edge_index_local: torch.Tensor, num_nodes: int) -> dict[int, set[int]]:
        ...
        Assumes directed edges as stored; bidirectional graphs will naturally
        have both directions if present in edge_index.
        """
    ...
    @staticmethod
    def get_existing_edge_feature(edge_index_local: torch.Tensor, edge_attr_local: torch.Tensor, source_local: int):
        """
    ...
    @staticmethod
    def copy_and_perturb_features(features: torch.Tensor, low: float = 0.97, high: float = 1.03) -> torch.Tensor:
    ...
    @staticmethod
    def append_spatial_edges(graph, source_global: int, target_global: int, edge_feature: torch.Tensor, mask):
        """
